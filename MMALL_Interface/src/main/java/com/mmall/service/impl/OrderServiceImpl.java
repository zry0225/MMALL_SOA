...
public class OrderServiceImpl implements OrderService {
    ...
    public ServerResponse pay(Long orderNo, Integer userId, String path) {
        ...
        if (order == null) {
            return ServerResponse.createByErrorMessage("用户没有这个订单");
        }
        String outTradeNo = orderNo.toString();
        // (必填) 订单标题，粗略描述用户的支付目的。如“xxx品牌xxx门店当面付扫码消费”
        String subject = "YGG品牌NO.1门店当面付消费,订单号为:" + orderNo.toString();
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                restMap.put("qrUrl",upload);
                restMap.put("orderNo",orderNo.toString());
                return ServerResponse.createBySuccess(restMap);
